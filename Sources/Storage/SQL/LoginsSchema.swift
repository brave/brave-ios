...
open class LoginsSchema: Schema {
  ...
  func run(_ db: SQLiteDBConnection, sql: String, args: Args? = nil) -> Bool {
    do {
      ...
      Log.legacy.error("Error running SQL in LoginsSchema: \(err.localizedDescription)")
      Log.legacy.error("SQL was \(sql)")
      return false
    }
  ...
  public func update(_ db: SQLiteDBConnection, from: Int) -> Bool {
    let to = self.version
    if from == to {
      Log.legacy.debug("Skipping update from \(from) to \(to).")
      return true
    }

    if from == 0 {
      // This is likely an upgrade from before Bug 1160399.
      Log.legacy.debug("Updating logins tables from zero. Assuming drop and recreate.")
      return drop(db) && create(db)
    }

    if from < 3 && to >= 3 {
      Log.legacy.debug("Updating logins tables to include version 3 indices")
      return self.run(db, queries: [indexIsOverriddenHostname, indexIsDeletedHostname])
    }

    // TODO: real update!
    Log.legacy.debug("Updating logins table from \(from) to \(to).")
    return drop(db) && create(db)
  }

  public func drop(_ db: SQLiteDBConnection) -> Bool {
    Log.legacy.debug("Dropping logins table.")
    do {
