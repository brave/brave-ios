...
class BraveTranslateScriptHandler: NSObject, TabContentScript {
  ...
  private func executePageFunction(name functionName: String) async -> String? {
    ...
    let (result, error) = await webView.evaluateSafeJavaScript(functionName: "window.__firefox__.\(BraveTranslateScriptHandler.namespace).\(functionName)", contentWorld: BraveTranslateScriptHandler.scriptSandbox, asFunction: true)
  ...
  func translatePage(pageSource: String, from fromLanguage: String?, to toLanguage: String?) async throws -> Any {
    guard var url = URL(string: "https://translate.bsg.brave.software/translate_a/t") as? NSURL else {
    ...
    func executeRequest(url: URL, method: String, headers: [String: String], body: String) async throws -> (Data, HTTPURLResponse) {
    ...
    let (data, _) = try await executeRequest(url: url as URL,
                                             method: "POST",
  ...
  func activateScript(from: String, to: String) {
    ...
    Task { @MainActor in
      await webView.evaluateSafeJavaScript(functionName: "window.__firefox__.\(BraveTranslateScriptHandler.namespace).start", contentWorld: BraveTranslateScriptHandler.scriptSandbox, asFunction: true)
    }
  }
  ...
  func userContentController(_ userContentController: WKUserContentController, didReceiveScriptMessage message: WKScriptMessage, replyHandler: @escaping (Any?, String?) -> Void) {
    ...
    guard let text = message["text"], 
          let encodedText = message["encoded"] else {
      Logger.module.error("Invalid Brave Translate Message")
      replyHandler(nil, nil)
      return
    }
    ...
    Task { @MainActor in
      do {
        ...
        if let result = result as? [String] {
          let prefix = text.prefix(while: { $0.unicodeScalars.allSatisfy(CharacterSet.whitespacesAndNewlines.contains) })
          let suffix = text.suffix(while: { $0.unicodeScalars.allSatisfy(CharacterSet.whitespacesAndNewlines.contains) })
