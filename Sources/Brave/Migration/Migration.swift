...
fileprivate extension Preferences {
  ...
  class func migrateAdAndTrackingProtection() {
    ...
    DeprecatedPreferences.blockAdsAndTracking.migrate(to: ShieldPreferences.blockAdsAndTrackingLevelRaw) { isEnabled -> String in
      if isEnabled {
        return ShieldLevel.standard.rawValue
      } else {
        return ShieldLevel.disabled.rawValue
      }
    }
...
private extension Preferences.Option {
  ...
  func migrate<Destination>(to destination: Preferences.Option<Destination>, transform: ((ValueType) -> Destination)) {
    // Have to do two checks because T may be an Optional, since object(forKey:) returns Any? it will succeed
    // as casting to T if T is Optional even if the key doesnt exist.
    let value = container.object(forKey: key)
    if value != nil, let value = value as? ValueType {
      destination.value = transform(value)
