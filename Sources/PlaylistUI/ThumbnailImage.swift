...
struct ThumbnailImage: View {
  ...
  @State private var renderer: PlaylistThumbnailRenderer = .init()
  ...
  var body: some View {
    Color.clear
      ...
      .task {
        image = await renderer.loadThumbnail(assetUrl: itemURL, favIconUrl: faviconURL)
      }
      .id(itemURL)
  }
}

extension PlaylistThumbnailRenderer {
  // FIXME: When moving PlaylistThumbnailUtility, add this version to it
  func loadThumbnail(assetUrl: URL?, favIconUrl: URL?) async -> UIImage? {
    let renderer = PlaylistThumbnailRenderer()
    return await withTaskCancellationHandler {
      await withCheckedContinuation { continuation in
        renderer.loadThumbnail(assetUrl: assetUrl, favIconUrl: favIconUrl) { image in
...
// --------------------- PlaylistThumbnailUtility.swift
