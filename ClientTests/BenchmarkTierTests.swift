...
class BenchmarkTierTests: XCTestCase {
    ...
    private func testNextTier(numberOfTrackerAds: Int) {
        let existingTierList = BrowserViewController.BenchmarkTrackerCountTier.allCases.filter({ numberOfTrackerAds < $0.rawValue })
    ...
    private var subject: [BrowserViewController.BenchmarkTrackerCountTier]!
}
