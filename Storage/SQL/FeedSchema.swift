...
open class FeedSchema: Schema {
    ...
    public func update(_ db: SQLiteDBConnection, from: Int) -> Bool {
        let to = self.version
        if from == to {
            log.debug("Skipping Feed schema update from \(from) to \(to).")
            return true
        }
    ...
    public func drop(_ db: SQLiteDBConnection) -> Bool {
        log.debug("Dropping Feed database.")
    ...
    func run(_ db: SQLiteDBConnection, sql: String, args: Args? = nil) -> Bool {
        do {
            ...
            log.error("Error running SQL in FeedSchema: \(err.localizedDescription)")
