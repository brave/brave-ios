...
extension BraveLedger {
  ...
  public func setupDeviceCheckEnrollment(_ client: DeviceCheckClient, completion: @escaping () -> Void) {
    ...
    client.generateToken { [weak self] (token, error) in
      guard let self = self else { return }
      if let error = error {
        Log.main.error("Failed to generate DeviceCheck token: \(error.localizedDescription)")
        completion()
        return
      }
      ...
      Task { @MainActor in
        do {
          ...
          Log.main.error("Failed to register device with mobile attestation server: \(error.localizedDescription)")
        }
        completion()
      }
    }
  ...
  public func claimPromotion(_ promotion: Ledger.Promotion) async -> Bool {
    ...
    if !DeviceCheckClient.isDeviceEnrolled() {
      ...
      if !didEnroll {
        Log.main.error("Cannot solve adaptive captcha as user is not enrolled with the attestation server")
        return false
      }
    }
