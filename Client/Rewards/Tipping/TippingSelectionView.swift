...
class TippingOption: NSObject {
  ...
  class func batAmount(_ value: BATValue, dollarValue: String) -> TippingOption {
    return TippingOption(
      value: value,
      crypto: "BAT",
      cryptoImage: UIImage(imageLiteralResourceName: "bat-small"),
...
class TippingSelectionView: UIView {
  ...
  var isMonthly: Bool = false {
    didSet {
      monthlyToggleButton.setImage(UIImage(imageLiteralResourceName: isMonthly ? "checkbox-checked" : "checkbox").alwaysOriginal, for: .normal)
  ...
  override init(frame: CGRect) {
    ...
    backgroundColor = RewardsPalette.blurple500
