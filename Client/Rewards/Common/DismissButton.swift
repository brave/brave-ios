...
class DismissButton: Button {
  ...
  override init(frame: CGRect) {
    ...
    setImage(UIImage(imageLiteralResourceName: "close-icon").alwaysTemplate, for: .normal)
    imageView?.tintColor = RewardsPalette.grey400
  ...
  override var isHighlighted: Bool {
    didSet {
      if isHighlighted {
        ...
        backgroundColor = RewardsBraveUX.braveOrange
      } else {
        imageView?.tintColor = RewardsPalette.grey400
        backgroundColor = UIColor(white: 1.0, alpha: 0.8)
      }
