...
class RewardsInternalsShareController: UITableViewController {
  ...
  private func cleanup(callingFromDeinit: Bool = false) {
    ...
    do {
      ...
      Log.main.warning("Failed to cleanup sharing Rewards Internals files: \(error.localizedDescription)")
    }
  }
    ...
    do {
      ...
      for sharable in sharables {
        ...
        sharable.generator.generateFiles(at: sharableFolder.path, using: builder) { error in
          defer { group.leave() }
          if let error = error {
            Log.main.error("Failed to generate files for the Rewards Intenrnals sharable with ID: \(sharable.id). Error: \(error.localizedDescription)")
          }
        }
      }
      group.notify(queue: .global(qos: .userInitiated)) { [weak self] in
        ...
        do {
          ...
          Log.main.error("Failed to zip directory: \(error.localizedDescription)")
          self.cleanup()
        }
      }
    } catch {
      Log.main.error("Failed to make temporary directory for rewards internals sharing: \(error.localizedDescription)")
      self.cleanup()
    }
  }
