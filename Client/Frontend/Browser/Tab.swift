...
class Tab: NSObject {
  ...
  var nightMode: Bool {
    didSet {
      ...
      webView?.evaluateSafeJavaScript(
        ...
        if let error = error {
          Log.main.error("Error executing script: \(error.localizedDescription)")
        }
      }
  ...
  func restore(_ webView: WKWebView, restorationData: SavedTab?) {
    ...
    if let sessionData = restorationData {
      ...
      Log.main.warning("creating webview with no lastRequest and no session data: \(String(describing: self.url))")
    }
  ...
  func injectResults() {
    DispatchQueue.main.async {
      ...
      self.webView?.evaluateJavaScript("window.onFetchedBackupResults === undefined") {
        ...
        if let error = error {
          Log.main.error("onFetchedBackupResults existence check error: \(error.localizedDescription)")
        }
