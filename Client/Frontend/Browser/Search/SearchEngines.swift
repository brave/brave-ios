...
public class SearchEngines {
  ...
  fileprivate lazy var customEngines: [OpenSearchEngine] = {
    do {
      ...
      Log.main.error("Failed to load custom search engines: \(error.localizedDescription, privacy: .public)")
      return []
    }
  }()
  ...
  fileprivate func saveCustomEngines() throws {
    do {
      ...
      Log.main.error("Failed to save custom engines: \(error.localizedDescription, privacy: .public)")
    }
  }
  ...
  private func createDefaultYahooCustomEngine(isJapan: Bool, forType type: DefaultEngineType) {
    ...
    if customMigrationEngineCreated == nil {
      do {
        ...
        Log.main.error("Search Engine migration Failed for \(engineDetails.engineName, privacy: .public)")
      }
    }
