...
class ErrorPageHandler: InternalSchemeResponse {
    ...
    func response(forRequest request: URLRequest) -> (URLResponse, Data)? {
        ...
        let tryAgain = Strings.errorPageReloadButtonTitle
        ...
        if errDomain == kCFErrorDomainCFNetwork as String {
            ...
            if errCode == MozErrorDownloadsNotEnabled {
                let downloadInSafari = Strings.errorPageOpenInSafariButtonTitle

                // Overwrite the normal try-again action.
                actions = "<button onclick='webkit.messageHandlers.errorPageHelperMessageManager.postMessage({type: \"\(MessageOpenInSafari)\", securitytoken:\"\(UserScriptManager.messageHandlerTokenString)\"})'>\(downloadInSafari)</button>"
            }
            errDomain = ""
            ...
            variables["security_token"] = UserScriptManager.messageHandlerTokenString
        }
    ...
    public static func certificateError(for url: URL) -> Int {
        if InternalURL.isValid(url: url),
           ...
           internalUrl.isErrorPage {
            ...
            if CertErrors.contains(errCode) {
                return errCode
            }
            
            if CertErrorCodes[errCode] != nil {
                return errCode
            }
            return 0
        }
