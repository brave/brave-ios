...
class CarplayMediaManager: NSObject {
    ...
    private var streamLoader = CarplayMediaManagerResourceManager({ _ in })
    ...
    private override init() {
        ...
        Playlist.shared.currentlyPlayingInfo.observe { [weak self] _, _ in
            self?.updateNowPlayingMediaItems()
        }
    ...
    public func updateNowPlayingMediaItems() {
        if let nowPlayingItem = Playlist.shared.currentlyPlayingInfo.value {
            ...
            MPNowPlayingInfoCenter.default().nowPlayingInfo = [
                ...
                MPNowPlayingInfoPropertyPlaybackRate: Double(self.playerView.player.rate),
                MPNowPlayingInfoPropertyPlaybackProgress: Float(0.0),
                MPNowPlayingInfoPropertyElapsedPlaybackTime: Double(self.playerView.player.currentTime().seconds)
            ]
        }
    }
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey: Any]?, context: UnsafeMutableRawPointer?) {
        if keyPath == "rate" {
            ...
            if let nowPlayingItem = Playlist.shared.currentlyPlayingInfo.value {
                contentManager.nowPlayingIdentifiers = [nowPlayingItem.name]
            }
    ...
    func playableContentManager(_ contentManager: MPPlayableContentManager, initiatePlaybackOfContentItemAt indexPath: IndexPath, completionHandler: @escaping (Error?) -> Void) {
        ...
        DispatchQueue.main.async {
            if indexPath.count == 2 {
                self.loadMediaItem(self.playlistItems[indexPath.row]) { error in
...
//            }
    ...
    public func loadMediaItem(_ item: PlaylistInfo, completion: @escaping (Error?) -> Void) {
        Playlist.shared.currentlyPlayingInfo.value = item
        ...
        if cache.isEmpty {
            if !item.src.isEmpty, let url = URL(string: item.src) {
                ...
                CarplayMediaManagerResourceManager.canStreamURL(url) {
                    if $0 {
                        ...
                        self.webLoader = PlaylistWebLoader(handler: { [weak self] newItem in
                            ...
                            if let newItem = newItem, let url = URL(string: newItem.src) {
                                ...
                                Playlist.shared.updateItem(mediaSrc: item.src, item: newItem) {
                                    DispatchQueue.main.async {
                                        if let index = self.playlistItems.firstIndex(where: { $0.src == item.src }) {
                                            self.playlistItems.remove(at: index)
                                            self.playlistItems.insert(newItem, at: index)
                                        }
                                        completion(nil)
                                    }
                                }
...
extension CarplayMediaManager: VideoViewDelegate {
    func onPreviousTrack() {
        if let currentItem = Playlist.shared.currentlyPlayingInfo.value,
            let index = playlistItems.firstIndex(where: { $0.src == currentItem.src }), index > 0 && index < playlistItems.count - 1 {
                self.loadMediaItem(playlistItems[index - 1]) { [weak self] error in
                    if error != nil {
...
class CarplayMediaManagerResourceManager: NSObject, AVAssetResourceLoaderDelegate {
    ...
    func resourceLoader(_ resourceLoader: AVAssetResourceLoader, shouldWaitForLoadingOfRequestedResource loadingRequest: AVAssetResourceLoadingRequest) -> Bool {
        ...
        if let contentRequest = loadingRequest.contentInformationRequest {
            ...
            guard CarplayMediaManagerResourceManager.isShimmedURL(requestURL) else { return false }
            let originalURL = CarplayMediaManagerResourceManager.unShimURL(requestURL)
...
extension CarplayMediaManagerResourceManager {
