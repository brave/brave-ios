...
struct ReaderModeHandlers {
    ...
    static func register(_ webServer: WebServer, profile: Profile) {
        ...
        webServer.registerHandlerForMethod("GET", module: "reader-mode", resource: "page") { (request: GCDWebServerRequest?) -> GCDWebServerResponse? in
                    ...
                    do {
                        ...
                        if let readerViewLoadingPath = Bundle.main.path(forResource: "ReaderViewLoading", ofType: "html") {
                            do {
                                ...
                                readerViewLoading.replaceOccurrences(of: "%LOADING-TEXT%", with: NSLocalizedString("LoadingContent", value: "Loading content…", comment: "Message displayed when the reader mode page is loading. This message will appear only when sharing to Firefox reader mode from another app."), 
                                    options: .literal, range: NSRange(location: 0, length: readerViewLoading.length))
                                readerViewLoading.replaceOccurrences(of: "%LOADING-FAILED-TEXT%", with: NSLocalizedString("ThePageCouldNotBeDisplayedInReaderView", value: "The page could not be displayed in Reader View.", comment: "Message displayed when the reader mode page could not be loaded. This message will appear only when sharing to Firefox reader mode from another app."),
                                    options: .literal, range: NSRange(location: 0, length: readerViewLoading.length))
                                readerViewLoading.replaceOccurrences(of: "%LOAD-ORIGINAL-TEXT%", with: NSLocalizedString("LoadOriginalPage", value: "Load original page", comment: "Link for going to the non-reader page when the reader view could not be loaded. This message will appear only when sharing to Firefox reader mode from another app."),
                                    options: .literal, range: NSRange(location: 0, length: readerViewLoading.length))
            ...
            let errorString = NSLocalizedString("ThereWasAnErrorConvertingThePage", value: "There was an error converting the page", comment: "Error displayed when reader mode cannot be enabled")
