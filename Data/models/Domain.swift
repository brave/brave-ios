...
public final class Domain: NSManagedObject, CRUD {
    ...
    public class func getForUrl(_ url: URL) -> Domain? {
        let domainString = url.domainURL.absoluteString
        return Domain.first(where: NSPredicate(format: "url == %@", domainString))
    }

    public class func getOrCreateForUrl(_ url: URL, context: NSManagedObjectContext? = nil, save: Bool = true) -> Domain {
    ...
    // MARK: - Shields
    public class func setBraveShield(forUrl url: URL, shield: BraveShield, isOn: Bool?, isPrivateBrowsing: Bool, context: NSManagedObjectContext? = nil) {
    ...
    public class func getBraveShield(forUrl url: URL, shield: BraveShield, isPrivateBrowsing: Bool,
                                     context: NSManagedObjectContext? = nil) -> Bool? {
        if isPrivateBrowsing {
            return getPrivateShieldForDomainUrl(url.domainURL.absoluteString, shield: shield)
        }
        let domain = Domain.getOrCreateForUrl(url)
